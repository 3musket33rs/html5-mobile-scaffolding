...
def show() {
    def $
    {classNameLowerCase} Instance = $ {className}.get(params.id)
...
def update() {
    ...
    < % if (oneToManyProps) {
        oneToManyProps.each {
            ...
            def referencedTypeToLowerCase = referencedType.toLowerCase()
            % >
            checkinReceived.$ {it.name} = $ {it.name}
            < %
        }
    } % >
    def $
    {classNameLowerCase} Instance = $ {className}.get(jsonObject.id)
    if (!$ {classNameLowerCase} Instance) {
        flash.message = message(code: 'default.not.found.message', args: [message(code: '${classNameLowerCase}.label', default: '${className}'), params.id])
        render flash as JSON
    }
...
def delete() {
    def $
    {classNameLowerCase} Id = params.id
    def $
    {classNameLowerCase} Instance = $ {className}.get(params.id)
    if (!$ {classNameLowerCase} Instance) {
        flash.message = message(code: 'default.not.found.message', args: [message(code: '${classNameLowerCase}.label', default: '${className}'), params.id])
